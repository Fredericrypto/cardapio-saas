...
interface DashboardDataValue {
  ...
  pendingVerifications: PendingVerificationItem[] | null;
  refetchOrders: () => void;
  refetchWaiterCalls: () => void;
  refetchActiveTables: () => void;
  refetchPendingVerifications: () => void;
}
...
// verificações pendentes — antes disso, o AdminLayout (pro blink do
// "Painel" no menu) e a PainelPage tinham CADA UM seu próprio polling
// pros mesmos endpoints, rodando em paralelo sem se coordenar. Além de
// dobrar as chamadas ao backend à toa, os dois relógios de polling
// ficavam fora de sincronia entre si (um atualizando a cada 4s, outro a
// cada 5s, cada um no seu próprio ciclo), o que podia fazer o piscar do
// card e o piscar do menu discordarem entre si por alguns segundos.
