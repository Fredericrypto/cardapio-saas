import type { Tenant, HistoryOrderEntry } from '../types';
...
// Mesmo estilo visual do ReceiptContent (cupom de mesa), mas pra um pedido
// avulso (balcão/entrega) — não existe "sessão" pra esses, é só o pedido.
export function ReceiptContentStandalone({
  tenant,
  order,
}: {
  tenant: Tenant;
  order: HistoryOrderEntry;
}) {
  ...
  return (
    <div className="font-mono text-xs text-gray-800 flex flex-col gap-2 w-full max-w-[280px] mx-auto">
      ...
      {order.tipAmount > 0 && (
        <div className="flex justify-between">
          <span>Gorjeta</span>
          <span>R$ {order.tipAmount.toFixed(2).replace('.', ',')}</span>
        </div>
      )}
      ...
      <div className="flex justify-between font-bold text-sm">
        ...
        <span>
          R${' '}
          {(Number(order.total) + Number(order.tipAmount)).toFixed(2).replace('.', ',')}
        </span>
      </div>
